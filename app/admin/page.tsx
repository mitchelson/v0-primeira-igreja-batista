...
import { Search, Plus, Check, User, AlertCircle, MessageSquare } from "lucide-react"
...
export default function AdminPage() {
  ...
  const [visitantesFiltrados, setVisitantesFiltrados] = useState<VisitanteComResponsavel[]>([])
  const [termoBusca, setTermoBusca] = useState("")
  const [visitanteSelecionado, setVisitanteSelecionado] = useState<VisitanteComResponsavel | null>(null)
  const [novoVisitanteDialogAberto, setNovoVisitanteDialogAberto] = useState(false)
  ...
  const carregarVisitantes = async () => {
    ...
    try {
      ...
      if (data) {
        ...
        const visitantesComResponsavel: VisitanteComResponsavel[] = data.map((item: any) => ({
          ...item,
    ...
    if (termoBusca.trim() === "") {
      ...
      const filtrados = visitantes.filter(
        (v) =>
          ...
          (v.responsavel_nome && v.responsavel_nome.toLowerCase().includes(termo)),
      )
  ...
  const handleVisitanteAtualizado = (visitanteAtualizado: Visitante) => {
    setVisitantes((prev) =>
      prev.map((v) =>
        v.id === visitanteAtualizado.id ? { ...visitanteAtualizado, responsavel_nome: v.responsavel_nome } : v,
      ),
    )
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <CardDescription>Gerencie os visitantes cadastrados</CardDescription>
            </div>
        ...
        <CardContent>
          ...
          {carregando ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              {termoBusca ? "Nenhum visitante encontrado para esta busca." : "Nenhum visitante cadastrado."}
            </div>
          ) : (
            <Tabs defaultValue={datasAgrupadas[0] || "todos"}>
      ...
      {novoVisitanteDialogAberto && (
        <NovoVisitanteDialog onClose={() => setNovoVisitanteDialogAberto(false)} onSave={handleNovoVisitante} />
      )}
    </div>
  )
}
