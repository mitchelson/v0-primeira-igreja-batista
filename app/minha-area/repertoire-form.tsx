...
export function RepertoireForm({
  eventoId,
  initialItems,
  onSaved,
  onCancel,
  ...
  const [items, setItems] = useState<RepertoireItem[]>(
    initialItems?.length
      ? initialItems
      : [{ nome: "", tonalidade: "", link: "", observacoes: "" }],
  );
  ...
  const handleSave = async () => {
    ...
    if (valid.length === 0) {
      toast({
        title: "Adicione pelo menos uma música",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          title: data.error || "Erro ao salvar",
          variant: "destructive",
        });
      }
    } finally {
        ...
        >
          ...
          <div className="flex gap-2">
            <Input
              placeholder="Tom"
              className="w-24"
              value={item.tonalidade}
              onChange={(e) => update(i, "tonalidade", e.target.value)}
            />
            <Input
